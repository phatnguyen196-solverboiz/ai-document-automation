...
export function DocumentPage() {
  ...
  useEffect(() => { Promise.all([api.getDocument(id), api.getExtraction(id).catch(() => null)]).then(([doc, data]) => { setDocument(doc); setExtraction(data); }).catch((err: Error) => setError(err.message)); }, [id]);
  async function runExtraction() { setBusy(true); try { await api.extract(id); navigate(`/documents/${id}/review`); } catch (err) { setError(err instanceof Error ? err.message : "Extraction failed"); } finally { setBusy(false); } }
  if (!document) return <div className="empty">{error || "Loading document…"}</div>;

  return <>
    <Link className="back-link" to="/">← Back to overview</Link>
    <section className="document-hero"><div><p className="eyebrow">SHIPPING DOCUMENT</p><h1>{document.original_filename}</h1><StatusBadge status={document.status} /></div><div className="hero-actions">{extraction ? <Link className="button" to={`/documents/${id}/review`}>Review extraction</Link> : <button className="button" disabled={busy} onClick={() => void runExtraction()}>{busy ? "Extracting…" : "Run extraction"}</button>}</div></section>
